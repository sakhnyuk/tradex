...
export const useCoreActions = () => {
  ...
  return useMemo(
    () => ({
      ...
      setShowAllPositions: (showAllPositions: boolean) => dispatch(act.setShowAllPositions(showAllPositions)),
      setShowAllPairs: (showAllPairs: boolean) => dispatch(act.setShowAllPairs(showAllPairs)),
      setOrderBookIsLoading: (orderBookIsLoading: boolean) => dispatch(act.setOrderBookIsLoading(orderBookIsLoading)),
      setTradesIsLoading: (tradesIsLoading: boolean) => dispatch(act.setTradesIsLoading(tradesIsLoading)),
      setPairListSorting: (pairListSorting: SortingPairs) => dispatch(act.setPairListSorting(pairListSorting)),
    }),
    [dispatch],
  );
};
